/*=========================================================
* Objetivo: Obter uma lista de livros;
* Data: 08/09/22
...
const getLivros = function(palavra){
    ...
    if(palavraChave != ""){
        livros.forEach(item => {
            item.books.forEach(livros => {
                if(livros.title.toLowerCase().includes(palavraChave.toLowerCase())){
                    livrosArray.push({Nome : livros.title, Subtitulo : livros.subtitle, Preço : livros.price, Imagem :  livros.image});
                    livrosJson.livros = livrosArray;
                    totalLivros++;
                    erro = false;
                }
            })
        });
        livrosJson.Total = totalLivros;
    }
    if(erro){
        return false;
    } else{
